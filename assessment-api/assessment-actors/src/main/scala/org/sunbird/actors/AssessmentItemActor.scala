  ...
  def update(request: Request): Future[Response] = {
    ...
    DataNode.read(request).flatMap(existingNode => {
      ...
      DataNode.update(request).map { node =>
        ResponseHandler.OK.putAll(Map("identifier" -> node.getIdentifier.replace(".img", ""), node.getIdentifier.replace(".img", ""), "versionKey" -> node.getMetadata.get("versionKey")).asJava)
      }
    })
  }
